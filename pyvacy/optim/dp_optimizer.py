...
#                         print( param.data)
            ...
            for group in self.param_groups:
                for param, accum_grad in zip(group['params'], group['accum_grads']):
                    if param.requires_grad:
                        accum_grad.add_(param.mul(clip_coef))
        ...
        def step(self, *args, **kwargs):
            for group in self.param_groups:
                for param, accum_grad in zip(group['params'], group['accum_grads']):
                    if param.requires_grad:
                        param = accum_grad.clone()
                        param.add_(self.l2_norm_clip * self.noise_multiplier * torch.randn_like(param))
                        param.mul_(self.microbatch_size / self.minibatch_size)
